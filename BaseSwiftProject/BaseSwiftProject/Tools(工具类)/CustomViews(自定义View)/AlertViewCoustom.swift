...
class AlertViewCoustom: NSObject, UIAlertViewDelegate, UIActionSheetDelegate {
    ...
    func showalertView(style:UIAlertController.Style, title:String?, message:String?, cancelBtnTitle:String?, touchIndex:@escaping ClickIndex, otherButtonTitles otherTitle:String?, _ moreButtonTitles: String...) {
        ...
        if #available(iOS 8.0, *) {
            ...
            if let cancelTitle = cancelBtnTitle {
                self.addAction(index: count, alertVC: alertController, style: UIAlertAction.Style.cancel, title: cancelTitle)
                count += 1
            }
            
            if let other = otherTitle, other.count > 0
            {
                self.addAction(index: count, alertVC: alertController, style: UIAlertAction.Style.default, title: other)
                count += 1
            }
            if moreButtonTitles.count > 0 {
                for otherIndex in 0 ..< moreButtonTitles.count
                {
                    self.addAction(index: count, alertVC: alertController, style: UIAlertAction.Style.default, title: moreButtonTitles[otherIndex])
                    count += 1
                }
            }
    ...
    func addAction(index:Int, alertVC:UIAlertController, style:UIAlertAction.Style, title:String) {
